"""Which values in a WhatsApp draft were written by hand.

A draft mixes two authors: the LLM, which rewrites the whole working set
on every turn, and the officer, who edits fields directly. Manual values
must survive every subsequent turn, so they are recorded as explicit field
paths and re-pinned server-side after the model answers.
"""
...
def _live_manual_fields(
    manual_fields: Iterable[str], previous: WhatsAppWorkingSet
) -> list[str]:
    ...
    `assign_row_ids` happened to hand its row_id to a later, unrelated row
    ...
    Session-field paths (no row_id, e.g. "as_at") are never row-scoped
...
def pin_manual_fields(
    next_working: WhatsAppWorkingSet, previous: WhatsAppWorkingSet
) -> WhatsAppWorkingSet:
    ...
    patch["incidents"] = _pin_rows(
        "incident", next_working.incidents, previous.incidents, manual, DraftIncident
    )
    patch["logs"] = _pin_rows(
        "log", next_working.logs, previous.logs, manual, DraftLog
    )
    return next_working.model_copy(update=patch)
