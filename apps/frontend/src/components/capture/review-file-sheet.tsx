...
export function detectDuplicateIncidents(
  ...
  for (const incident of incidents) {
    ...
    if (summary) {
      bySummary.set(summary, [
        ...(bySummary.get(summary) ?? []),
        incident.row_id,
      ])
    }
...
export function ReviewFileSheet({
  ...
  const blockedNoEvent =
    session.event_id === null && session.incidents.length > 0
        ...
        <div className="min-h-0 flex-1 space-y-5 overflow-y-auto px-4 py-4">
          <dl className="grid gap-3 text-sm sm:grid-cols-3">
            ...
            <div>
              ...
              <dd className="tabular-nums">{formatWhen(session.as_at)}</dd>
            </div>
          </dl>

          <p className="text-sm font-medium">
            {session.incidents.length} incident
            {session.incidents.length === 1 ? '' : 's'} · {session.logs.length}{' '}
            log{session.logs.length === 1 ? '' : 's'}
          </p>

          {duplicateRowIds.size > 0 ? (
            <div className="rounded-md border border-destructive/50 bg-destructive/5 px-3 py-2 text-sm text-destructive">
              {duplicateRowIds.size} incident
              {duplicateRowIds.size === 1 ? '' : 's'} look
              {duplicateRowIds.size === 1 ? 's' : ''} like duplicates — check
              before filing
            </div>
          ) : null}
                ...
                <p className="text-muted-foreground">
                  {[
                    ...
                    incident.incident_type
                      ? formatConstant(incident.incident_type)
                      : null,
                  ]
                    .filter(Boolean)
                    .join(' · ') || '—'}
          ...
          {session.missing.length > 0 ? (
            <div className="rounded-md border border-amber-500/50 bg-amber-500/5 px-3 py-2 text-sm">
              <p className="font-medium">
                {session.missing.length} detail
                {session.missing.length === 1 ? '' : 's'} still needed
              </p>
              ...
              <p className="mt-1 text-muted-foreground">
                Filing is still allowed with these gaps -- they can be filled in
                later.
              </p>
            </div>
          ) : null}
